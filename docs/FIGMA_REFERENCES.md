...
> 이 문서는 `docs/ARCHITECTURE.md`, `docs/BLOCKCHAIN_DESIGN.md`, `docs/TABS_FEATURES.md`와 함께,
...
| 토큰 | Hex | 용도 |
|---|---|---|
| violet-600 | `#7c3aed` | **주 액션** (연결/요청/활성 탭/연도·월 선택 활성) |
...
| 인사 출처 | sky(렌탈리스트 인수 대기·0016 이후 신규 미발생) / emerald(입력됨) | 인사 정보 관리 |
...
- **청구달력**: 연도 버튼 그룹(violet-600 활성 / Outline 비활성) → 테이블 `min-w-[1200px]`
  - 컬럼: 자산(관리번호 mono + 모델명) · 1월~12월(각 `min-w-76px`) · 합계.
  - 셀: 납부 금액 mono(우측), 0원이면 `—`, 계약 종료 후 연도면 `만기`(amber), **현재 월 컬럼** =
    **violet-400 보더 + 헤더·푸터 `bg-violet-500/10` + 본문 셀 `bg-violet-500/5`**, 헤더 텍스트 violet-200.
  - 합계 행(중간 `border-t-2 border-neutral-600 bg-neutral-800/40`): 월별 합계(violet-300) + `₩총합계`(amber-300).
- **월간리포팅**: 상단 컨트롤 [◀] [YYYY년 MM월 — 활성 `bg-violet-600 text-white`, 비활성 Outline] [▶]
  + **요약 카드 2종**(`sm:grid-cols-2`): "확인 필요 (인사·근무 기준)" `amber-300` N건 /
  "기한 경과 포함 · 만기(향후 3개월) 도래" `rose-300` N건(현재월이 아니면 "{월} 리포팅 기준 기한 경과·만기 도래")
  → 하단 두 테이블(①·②).
  - 헤더 설명: "내가 관리하는 자산 기준 — 인사상태·근무위치로 확인이 필요한 자산과 만기(계약 종료) 도래 자산…"

### B1-1. 월간리포팅 ① 확인 필요 자산
- 컬럼: 사유(배지) · 관리번호(mono, 링크 없음) · 모델명(+카테고리 10px neutral-500) · 사용자 · 부문/팀 · 자산위치 · 인사상태 · 근무위치 · 입사일 · 퇴직(예정)일(+`N일 남음` — 10px neutral-500) · 권장조치.
- 사유 배지 색은 A3 표 참조. 정렬: 관리번호 내림차순.
- 사유 = 퇴직 / 퇴직(전근)예정 / 신규 입사 / 휴직·출산휴가·육아휴직 / 파견 근무 / **자산-근무지 위치 상이**(자산위치≠근무위치, violet) / 수습 / 기타. (위치 판정 로직은 C4 참조)

### B1-2. 월간리포팅 ② 만기 도래 자산
- 컬럼(순서 유의): **남은 기간**(첫 열) · 관리번호 · 모델명(+카테고리) · 사용자 · 부문/팀 · 자산위치 · 종료일 · 권장 조치.
- 남은 기간 배지: `기간 경과`(red, days_left<0) / `당일`(0) 또는 `D-N`(≤30 → amber, 초과 → neutral).
- 조치: 기한 경과 → **"반납·연장 대책 필요 (기한 경과)"** red 강조 / 이내 → "재계약(연장)·반납 검토".
- 정렬: 렌탈 종료일 오름차순.
...
- 리스트 테이블 `min-w-[1220px]` 컬럼: 관리번호(+이관 그래프 ↗ 링크) · 카테고리(neutral 칩) · 모델명(+SN `10px mono`) · 제조사 · 사용자 · 부문/팀 · 자산위치 · 렌탈사 · 청구(주기+청구월 10px) · 렌탈료(`1,000원` 형식) · 렌탈 시작일 · 렌탈 종료일 · 상태(배지) · 인수인계(액션).
- **정렬 규칙**: 이전 흐름에 관여된(내게 온/내가 요청한) 자산 최상단.
- **인수인계 액션 셀·상태 열 연동** (상태 열은 다음 표 우측):
  - 일반: [WalletDirectoryPicker(지갑 선택 입력)] + [이전 요청] violet 버튼 — 상태 열 = 상태 배지
  ...
  - 인입·미결의: `내게 이전 요청`(blue) + [인수 승인](emerald) + [인수 거절](outline red)
  - 인입·승인: `인수 승인 완료`(amber) — 상태 열 = `관리자 승인 대기`
  - 인입·거절: `인수 거절됨`(red)
  - 계약종료: `—`
- **자산 등록 모달** (A4 모달): 입력 — 관리번호* · 시리얼 · 모델명* · 카테고리(select) · 제조사 · 사용자(**인사 프로필 select** — `/api/hr-people`, 부문/팀 자동 채움) · 부문/팀(수정 가능) · 자산위치(예: B-XX지사) · 렌탈사 · 청구(select) · 렌탈료(숫자) · 청구월 · 시작일 · 종료일 · 상태. 하단 [등록 완료]/[취소]. 미등록 인원은 서버에서 400 거부(0016 FK, 인사 정보 관리에서 먼저 등록).

## B4. 유휴 자산(전사) (idle) — 전체 사용자 공개
- 큐: 제목 "유휴 자산 (전사 공개)" + 설명 → AssetFilterBar → 테이블 `min-w-[1100px]`.
- 컬럼: 관리번호 · 카테고리 · 모델명(+SN) · 제조사 · 사용자 · 부문/팀 · 자산위치 · 렌탈사 · 청구 · 렌탈료 · 렌탈 시작일 · 렌탈 종료일 · **상태(유휴만)**.
...
- **배치 바** (진행 요청 존재 시): [전체 선택/해제] [이전 요청 N건 중 M건 선택] [선택 M건 승인(emerald)] [선택 M건 거절(red)] + 캡션 "승인은 수신자 승인이 완료된 항목만 처리되고, 여러 건을 한 트랜잭션에 묶어 수수료를 절감합니다".
- 테이블 컬럼 (min-width): 선택(체크박스) · 관리번호`7rem`(+이관 그래프) · 카테고리`7rem`(select) · 모델명`12rem`(읽기) · **사용자`8rem`(인사 select)** · 부문`8rem`(input) · 팀`9rem`(input) · 자산위치`8rem`(input) · 렌탈사`10rem`(읽기) · 렌탈료`7rem`(숫자 input) · 시작일`9rem`(date) · 종료일`9rem`(date) · 상태`7rem`(select) · 이전 요청`14rem`(배지) · 이관 기록`10rem`.
- **사용자 select**: 옵션 = 인사 정보 관리 목록(`/api/employees`) 사람들 + `—`(비움). 현재 값이 목록 밖이면 대비 보완 옵션. **분·팀은 인사 선택 시 자동 동기화**.
- **체크박스 활성 조건**: 진행 중 이전 요청(`pending_to_wallet`)이고 수신자 미거절·관리자 미승인일 때만 — 실행은 배치 바 [선택 M건 승인/거절]으로 수행(행 단위 단건 버튼 없음).
- **일괄 실행 대상**: 승인 = 수신자 승인 완료 + `정상사용`만(그 외는 배치 응답에서 `제외`) / 거절 = 모든 진행 요청.
- **이전 요청 셀**: 배지 `수신자 승인 대기 → 주소`(amber) / `수신자 승인 → 주소`(emerald) / `수신자 인수 거절 → 주소`(red) — 주소는 축약 표기 + 옆에 요청시각(10px neutral-500), 요청 없으면 `—`.
- **이관 기록**: Explorer 링크(`explorer.solana.com/tx/{tx}`, devnet 쿼리) `transfer_tx` mono 축약 + [이관 그래프 ↗] 링크 / 없으면 `—`.
- 정렬: 진행 중 이전(수신자 승인 대기 → 수신자 승인) 최상단, 이후 관리번호 내림차순.
- 편집 변경은 PATCH로 즉시 저장, row 플래시(5초) 피드백.

## B6. 인사 정보 관리 (hr) — 관리자 전용
- 외곽 폭 `max-w-[1400px]`. 큐: [제목+검색(이름/부문/팀)] → [새 프로필 등록 행] → 테이블 `min-w-[1200px]`.
- 제목 옆 캡션: "월간 리포팅(퇴직·휴직·신규입사·위치·만기 예고)의 판단 기준".
- 등록 행: 텍스트 input(placeholder "렌탈리스트 외 사용자 이름으로 새 인사 프로필 추가") + [등록] emerald.
- 테이블 컬럼: 이름 · 출처(배지: sky `렌탈리스트 인수 대기` / emerald `입력됨`) · 부문/팀(select 2개: 부문 `A부문/B부문/C부문`, 팀 `AAAA팀~FFFF팀` + `—` 비움; 목록 밖 저장 값은 보완 옵션) · 인사상태(select: 재직/수습/휴직/출산휴가/육아휴직/파견/퇴직/기타) · 근무위치(select: 본사/지사/재택/해외지사/출장중) · 직급(input) · 입사일(date) · 퇴직(예정)일(date) · 비고(input) · 저장(버튼, 역할/저장 중 상태 포함).
...
3. 찰리: 최종 승인 → ① 서비스 지갑이 `create_handover(asset_id, asset_code)` 계정 `{from, to, fee_payer}`로 온체인 실행(1서명) + `HandoverCreated` 이벤트 ② `transfer_tx` 기록 ③ DB `managed_by → Bob` ④ `transfer_history` 삽입.
- 분기: 밥 거절 시 → 찰리 승인 불가(붉은 X) / 같은 요청 두 번 → 멱등(조건부 업데이트).
- 상태 초기화 구분: 관리자 거절 → `pending_rejected_at` 기록 / 수신자 거절 → `pending_receiver_rejected_at` 기록 / 담당자 취소(DELETE) → **흔적 없이** 요청 전 상태로 복원.
...
2. 우선순위 체크(좌→우로 첫 매치):
퇴직(red) → 퇴직예정(3개월 윈도우, orange) → 신규입사(당월 hire, emerald) → 휴직·출산·육아(amber) → 파견(violet) → **자산위치 카테고리 ≠ 근무위치** → 자산-근무지 위치 상이(violet) → 수습(cyan) → 기타(neutral).
3. 별도로 만기 도래: `rental_end_date ≤ +3개월 말`(과거 포함) → days_left<0 `기한 경과(red)` / 0 `당일(amber)` / ≤30 `D-N(amber)` / 그 외 `D-N(neutral)`.
- 자산위치 카테고리 매핑(우선순위): 재택→재택, 해외→해외지사(지사보다 우선), 지사→지사, 본사/사옥/HQ→본사, 그 외→비교 제외.
...
# D. 아키텍처 다이어그램 (설계 문서 이미지용)

피그마 캔버스(와이드 1600~2000px)로 제작해 `BLOCKCHAIN_DESIGN.md`·포트폴리오에 삽입.
...
- 환경: Mainnet/Devnet 분기(`NEXT_PUBLIC_SOLANA_NETWORK`), `FEE_PAYER_SECRET`(서버 전용 키)로 서비스 지갑 서명.
...
1. **D1 + D4** (더블-레저, 배치 수수료) — 차별점 1순위, 설계 문서 이미지로 바로 사용.
